# TRI-CHEF: Complex-Hermitian Embedding Fusion for Korean Multimodal Retrieval — 시스템 통합 문서

> 생성일: 2026-04-24 | 갱신: 2026-04-27 14:20 | **v1-6 paper 동기화: 2026-04-27**
> 통합 대상: `TRICHEF_PIPELINE_AND_CONCEPTS.md` · `TRICHEF_PIPELINE.md` · `TRICHEF_SPECIFIC.md` (모두 2026-04-27 삭제 — 본 문서로 흡수 완료)
...
> **v1-6 추가** (2026-04-27): MIRACL-ko 자체 재현 nDCG@10=77.82 (BGE-M3 published 69.9 대비 +7.92pp, 정확 FAISS IndexFlatIP) · MR Music 파이프라인 RTX 4070 최적화 (모델 1회 로드, BGE-M3 batch 16→64, ~3.5GB VRAM) · `run_index.py` UTF-8 강제 (cp949 UnicodeEncodeError 해결) · 태윤_2차 47개 음원 파일 인덱싱 (AI/게임/뉴스/동물/음식/음악/일상 7개 카테고리) · Admin UI 이모지 제거 + 파이프라인 섹션 collapsible 기능
...
TRI-CHEF(Triple-Channel Complex-Hermitian Embedding Fusion)는 **이미지 · 문서 · 영상 · 음악** 4개 도메인을 단일 API로 통합하는 멀티모달 벡터 검색 시스템이다.

> **약어 풀이**: TRI = Triple-Channel, CHEF = **C**omplex-**H**ermitian **E**mbedding **F**usion, ASF = Adaptive Sieve Filter
...
## 4. CHEF — Complex-Hermitian Embedding Fusion 핵심 개념
...
| **Fusion** | 다중 모달·다중 채널을 단일 점수로 융합하는 통합 체계 (CHEF 두문자어의 F) |
...
| BGE-M3 dense (published) | 69.9 | — | — | BGE-M3 arXiv:2402.03216 (ANN) |
| **Tri-CHEF (Im axis) — 자체 재현 (v1-6)** | **77.82** | **95.46** | **76.56** | ours, FAISS IndexFlatIP, RTX 4070, seed 2026 |

**해석 (v1-6 갱신)**:
- **자체 재현값 77.82**는 BGE-M3 published 수치(69.9, ANN 기반) 대비 **+7.92pp** 우위. 차이 원인은 ANN 근사 대신 정확 FAISS `IndexFlatIP`(brute-force) 검색을 사용했기 때문이며, 인코더 가중치/시드는 동일.
- BM25 대비 **+40.72pp**, mDPR 대비 **+35.92pp**, mContriever 대비 **+29.52pp**, mE5-large-v2 대비 **+11.32pp** — 전 baseline 압도.
- 한국어 dense 검색이 BM25 대비 명확히 성숙했음을 확인하며, Tri-CHEF가 Im축 백본으로 BGE-M3를 선택한 정당성을 외부 벤치마크에서 검증.
- Re축(SigLIP2)과 Z축(DINOv2)은 이 한국어 텍스트 기초 위에 cross-modal 및 시각 구조 증거를 추가하는 역할.

**재현 환경**: 단일 NVIDIA GeForce RTX 4070 (8GB VRAM), 정확 FAISS IndexFlatIP, batch_size=128, max_length=1024, random seed 2026. 결과 파일: `bench_results/miracl_ko_bgem3_20260427_023514.json`.

**자체 재현 가능성**: `scripts/eval_miracl_ko.py` 및 baseline 구현체(`scripts/baselines/{bm25,mdpr,mcontriever,me5,bgem3}.py`)를 통해 결과 재현 가능. 단일 명령: `bash scripts/run_all_miracl_baselines.sh --top-k 100 --batch-size 64` (RTX 4070에서 약 40~60분).
