...
class GroundingGate:
    ...
    def check(
        ...
        # 요약 질의 + 문서 지시 토큰이 검색 청크에 명확히 매칭되면 통과.
        # (질문이 지시형일 때 코사인 유사도 과소평가로 차단되는 문제 완화)
        if _is_summaryish_query(user_query):
            tok = _extract_hard_doc_token(user_query)
            if tok:
                tok_norm = _norm_kr(tok)
                for c in chunks:
                    meta_blob = " ".join([
                    ...
                    if tok_norm and tok_norm in _norm_kr(meta_blob):
                        logger.info(
                            "GroundingGate: summary 토큰 매칭(%r)으로 통과(메타)",
                            tok,
                        )
                        return True
                for c in chunks:
                    if tok_norm and tok_norm in _norm_kr(c.get("text") or ""):
                        logger.info(
                            "GroundingGate: summary 토큰 매칭(%r)으로 통과(본문)",
                            tok,
                        )
                        return True
                # 문서 지시 토큰이 있으나 매칭이 불안정할 때:
                # 요약 후보가 사실상 단일 문서라면 fallback으로 통과
                # (벡터 검색의 고유명사 미매칭 완화)
                doc_keys = set()
                for c in chunks:
                    key = " ".join([
                    ...
                    if key:
                        doc_keys.add(_norm_kr(key))
                if len(doc_keys) == 1:
                    logger.info(
                        "GroundingGate: summary 단일문서 fallback 통과(token=%r)",
                        tok,
                    )
                    return True
