  ...
  return (
    <div className="flex flex-col flex-1 gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-on-surface-variant">
          .docx · .hwp · .pptx · .xlsx 파일 처리를 위해 LibreOffice가
          필요합니다.
        </p>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-surface-container-high border border-outline-variant/15">
        ...
        {(installing || error) && (
          <div className="space-y-2">
            ...
            <p
              className={`text-xs ${error ? "text-red-400" : "text-on-surface-variant"} truncate`}
            >
              {error || message || "설치 중..."}
            </p>
          </div>
      ...
      <div className="space-y-2 text-base text-on-surface-variant/70">
        {[
          ...
          <div key={i} className="flex items-start gap-2">
            <span className="material-symbols-outlined text-base text-primary/60 mt-0.5 shrink-0">
              info
            </span>
            <span>{t}</span>
          </div>
      ...
      {error && (
        <p className="text-xs text-center text-red-400/80">
          ...
          <span
            ...
            onClick={() =>
              window.open?.("https://www.libreoffice.org/download/")
            }
          >
            libreoffice.org
          </span>
      ...
      <main className="relative z-10 flex min-h-screen flex-col items-center justify-center p-6 pt-10">
        <div className="relative isolate w-full max-w-4xl">
          ...
          <div className="relative min-h-[36rem] overflow-hidden rounded-2xl shadow-[0_28px_80px_rgba(0,0,0,0.55)] lg:min-h-[34rem]">
            <div
              className="pointer-events-none absolute inset-0 app-depth-bg"
              aria-hidden
            />
              ...
              <section className="relative flex flex-col items-center justify-center gap-8 overflow-visible border-b border-white/10 px-8 py-12 lg:border-b-0 lg:border-r lg:px-10">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="mt-3 max-w-sm text-sm leading-relaxed text-white/65">
                    마스터 비밀번호로 개인 데이터를 보호하세요. 모든 처리는
                    기기에서 이루어집니다.
                  </p>
                </div>
                ...
                <div className="mx-auto w-full max-w-md flex flex-col h-full">
                    ...
                    <div className="flex gap-2">
                      {["마스터 키"].map((label, i) => (
                        <div key={i} className="flex items-center gap-1.5">
                          <div
                            className={`h-1 w-10 rounded-full transition-all duration-500 ${i <= stepIndex ? "bg-gradient-to-r from-sky-300/90 to-[#2563eb]" : "bg-white/15"}`}
                          />
                          <span
                            className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${i === stepIndex ? "text-sky-200" : "text-white/30"}`}
                          >
                            {label}
                          </span>
                        </div>
                  ...
                  {step === "password" && (
                    <form onSubmit={handleSubmit} className="space-y-8 flex-1">
                      <div>
                        <h2 className="mb-2 text-xl font-bold tracking-tight text-white md:text-2xl">
                          마스터 비밀번호 생성
                        </h2>
                        <p className="text-sm text-white/60">
                          마스터 비밀번호는 모든 데이터 저장소를 보호합니다.
                        </p>
                      </div>
                      {error && (
                      ...
                      <button
                        type="submit"
                        className="group mx-auto flex h-14 w-fit items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#060d1f] via-[#0f2847] to-[#2563eb] px-8 font-bold tracking-tight text-white shadow-[0_4px_24px_rgba(37,99,235,0.35)] transition-all hover:shadow-[0_8px_36px_rgba(56,189,248,0.35)] hover:brightness-[1.05] active:scale-[0.98]"
                      >
                        비밀번호 생성
        ...
        <div className="mt-12 flex flex-wrap items-center justify-center gap-3">
          <div className="flex items-center gap-2 rounded-full bg-white/[0.06] px-4 py-2 backdrop-blur-md">
            <span
              className="material-symbols-outlined text-xs text-sky-200/90"
              style={{ fontVariationSettings: '"FILL" 1' }}
            >
              verified_user
            </span>
