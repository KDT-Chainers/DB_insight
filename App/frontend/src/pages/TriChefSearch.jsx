...
export default function TriChefSearch() {
  ...
  async function runReindex() {
    if (
      !confirm(
        "전체 재임베딩을 시작합니다. 시간이 오래 걸릴 수 있습니다. 계속하시겠습니까?",
      )
    )
      return;
    setLoading(true);
      ...
      >
        <div className="p-6 max-w-5xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-black text-[#dfe4fe]">
                TRI-CHEF 통합 검색
              </h1>
              <p className="text-xs text-on-surface-variant mt-1">
                3종 복소 벡터 검색 · SigLIP2 + e5-large + DINOv2
              </p>
            </div>
            <button
              onClick={runReindex}
          ...
          <div className="flex gap-2 mb-6">
            <input
              ...
              placeholder="자연어로 검색하세요 (예: 쇼핑몰, 매출 차트)"
              ...
              {Object.entries(stats.per_domain).map(([d, s]) => (
                ...
                >
                  ...
                  {s.error ? (
                    ...
                    <span className="ml-2">
                      {s.count}건 · μ={s.mu_null} · σ={s.sigma_null} · thr=
                      {s.abs_threshold}
                    </span>
                  )}
                </div>
