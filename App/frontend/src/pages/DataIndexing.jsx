    ...
    children: [
      {
        ...
        expanded: false,
        children: [
          { id: 'f1', name: 'neural_architecture.json', type: 'file', icon: 'description', status: 'Ready', statusColor: 'text-primary bg-primary/10', checked: true },
          { id: 'f2', name: 'dataset_v04_raw.csv', type: 'file', icon: 'table_chart', status: 'Pending', statusColor: 'text-secondary bg-secondary/10', checked: false },
          {
            id: 'logs',
            name: 'processed_logs',
            type: 'folder',
            ...
            expanded: true,
            children: [
              { id: 'l1', name: 'node_trace_01.log', type: 'file', icon: 'analytics', status: 'Ready', statusColor: 'text-primary bg-primary/10', checked: true },
              { id: 'l2', name: 'node_trace_02.log', type: 'file', icon: 'analytics', status: 'Corrupt', statusColor: 'text-error bg-error/10', checked: false },
            ],
          },
        ],
      },
...
export default function DataIndexing() {
  ...
  const [complexityMode, setComplexityMode] = useState('Deep Learning')
      ...
      <aside className="h-screen w-64 sticky left-0 top-0 bg-[#000000] flex flex-col py-6 shadow-2xl shadow-blue-900/20 bg-gradient-to-b from-[#070d1f] to-[#000000] z-50">
        <div className="px-6 mb-10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-manrope uppercase text-[10px] tracking-widest text-[#a5aac2]">Intelligence</p>
          </div>
        </div>
        <nav className="flex-1 space-y-1 px-4">
          <button onClick={() => navigate('/search')} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#0c1326] hover:text-[#dfe4fe] transition-all group text-[#a5aac2]">
            ...
            <span className="font-manrope uppercase text-[10px] tracking-widest">Workspace</span>
          </button>
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#0c1326] hover:text-[#dfe4fe] transition-all group text-[#a5aac2]">
            ...
            <span className="font-manrope uppercase text-[10px] tracking-widest">Data Sources</span>
          </button>
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#1c253e] text-[#85adff] border-r-2 border-[#85adff]">
            <span className="material-symbols-outlined">account_tree</span>
            <span className="font-manrope uppercase text-[10px] tracking-widest">Indexing</span>
          </div>
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#0c1326] hover:text-[#dfe4fe] transition-all group text-[#a5aac2]">
            <span className="material-symbols-outlined transition-transform group-hover:translate-x-1">memory</span>
            <span className="font-manrope uppercase text-[10px] tracking-widest">Vector Store</span>
          </button>
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#0c1326] hover:text-[#dfe4fe] transition-all group text-[#a5aac2]">
        ...
        <div className="px-4 mt-auto space-y-4">
          ...
          <div className="pt-4 border-t border-[#41475b]/20 space-y-1">
            <button onClick={() => navigate('/settings')} className="w-full flex items-center gap-3 px-4 py-2 text-[#a5aac2] hover:text-[#dfe4fe] transition-colors">
              ...
              <span className="font-manrope uppercase text-[10px] tracking-widest">Security</span>
            </button>
            <button className="w-full flex items-center gap-3 px-4 py-2 text-[#a5aac2] hover:text-[#dfe4fe] transition-colors">
              <span className="material-symbols-outlined text-sm">sensors</span>
              <span className="font-manrope uppercase text-[10px] tracking-widest">Status</span>
            </button>
          </div>
        </div>
      </aside>
        ...
        <header className="w-full top-0 sticky bg-[#070d1f]/60 backdrop-blur-xl flex justify-between items-center px-6 h-16 border-b border-[#41475b]/15 shadow-[0_0_40px_rgba(133,173,255,0.1)] z-40">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-6">
              {['Explorer', 'Neural Map', 'History', 'Clusters'].map((item, i) => (
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-container-low border border-outline-variant/30">
              ...
              <input className="bg-transparent border-none focus:ring-0 text-sm w-48 text-on-surface-variant placeholder-[#41475b] outline-none" placeholder="Neural search..." />
            </div>
        ...
        <div className="flex-1 p-8 flex flex-col gap-6 overflow-hidden">
          ...
          <div className="grid grid-cols-12 gap-6 items-center">
            <div className="col-span-12 lg:col-span-7 glass-panel p-6 rounded-xl border border-outline-variant/15 flex items-center justify-between gap-6">
              <div className="flex-1">
                <label className="text-[10px] uppercase tracking-[0.1em] text-primary mb-2 block font-bold">Configuration Path</label>
              ...
              <button className="px-6 py-3 rounded-full bg-surface-container-high border border-outline-variant/40 hover:bg-surface-container-highest transition-all text-sm font-semibold flex items-center gap-2 whitespace-nowrap">
                <span className="material-symbols-outlined text-sm">folder_shared</span>Select Folder
              </button>
            </div>
            <div className="col-span-12 lg:col-span-5 glass-panel p-6 rounded-xl border border-outline-variant/15">
              <label className="text-[10px] uppercase tracking-[0.1em] text-secondary mb-4 block font-bold">Neural Parameters</label>
              <div className="flex flex-col gap-4">
                {/* Index mode toggle */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-on-surface-variant">Indexing Mode</span>
                ...
                {/* Complexity toggle */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-on-surface-variant">Complexity Engine</span>
                  <div className="flex bg-surface-container-lowest rounded-full p-1 border border-outline-variant/20">
                    {['Basic', 'Deep Learning'].map((mode) => (
                      <button
                        key={mode}
          ...
          <div className="flex-1 glass-panel rounded-xl border border-outline-variant/15 overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-outline-variant/10 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <h2 className="text-lg font-bold tracking-tight text-on-surface">Resource Explorer</h2>
                <span className="px-3 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-bold tracking-widest uppercase">142 Items found</span>
              </div>
              <div className="flex items-center gap-2">
                <button className="text-xs font-semibold text-on-surface-variant hover:text-primary transition-colors">Expand All</button>
                <span className="w-px h-3 bg-outline-variant/30"></span>
                <button className="text-xs font-semibold text-on-surface-variant hover:text-primary transition-colors">Collapse All</button>
              </div>
            </div>
            ...
            <button className="relative group">
              ...
              <div className="relative px-12 py-5 bg-[#000000] rounded-full leading-none flex items-center divide-x divide-outline-variant/30">
                <span className="flex items-center space-x-5">
                  ...
                  <span className="pr-6 text-on-surface font-black tracking-widest text-sm uppercase">Start Indexing Selected Files</span>
                </span>
                <span className="pl-6 text-secondary text-xs font-bold uppercase tracking-widest">84.2 GB Target</span>
              </div>
            </button>
          </div>
        </div>
