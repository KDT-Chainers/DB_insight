        ...
        <div className="flex gap-0 flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            {isRunning && runningResult?.step != null && (
              <div className="shrink-0 border-b border-outline-variant/20 px-6 pb-5 pt-6">
                        ...
                        className={`relative flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all duration-300 ${
                          isActive
                            ...
                            : isPast
                              ...
                              : 'border border-outline-variant/15 bg-surface-container-high/30'
                        }`}>
                        {/* 번호 / 아이콘 */}
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-black border-2 transition-all ${
                          isActive
                            ...
                            : isPast
                              ...
                              : 'border border-outline-variant/20 bg-surface-container-high/40 text-on-surface-variant/30'
                        }`}>
                          {isPast
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="di-glass-card rounded-2xl border border-outline-variant/20 p-5">
        <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-secondary">ChromaDB 정보</p>
...
export default function DataIndexing() {
  ...
  return (
    <div className="app-depth-bg flex h-screen overflow-hidden text-on-surface">
      ...
      {/* Sidebar */}
      <aside className="di-glass-shell z-50 flex h-screen w-64 shrink-0 flex-col rounded-r-3xl pt-12 pb-4 shadow-[20px_0_48px_rgba(133,173,255,0.06)]">
        ...
        <nav className="flex-1 space-y-1.5 px-3">
          {[
            ...
            const isCat = item.tab != null
            const active = isCat && item.tab === tab
            return (
            <button
              key={item.label}
              onClick={item.onClick ?? (() => setTab(item.tab))}
              className={`flex w-full items-center gap-3 rounded-2xl px-4 py-2.5 text-[0.75rem] font-manrope uppercase tracking-widest transition-all
                ${active
                  ? 'di-nav-active text-on-surface'
                  : 'border border-transparent text-on-surface-variant hover:bg-surface-container-high/70 hover:text-on-surface'}`}
            >
              <span className={`material-symbols-outlined shrink-0 text-base ${active ? 'text-primary' : ''}`}>{item.icon}</span>
              {item.label}
            </button>
            )
          })}
        ...
        {selectedCount > 0 && (
          <div className="di-glass-card di-glass-card--accent mx-3 mb-3 rounded-2xl border border-primary/25 p-3">
            <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-primary">선택됨</p>
            <p className="text-xl font-black leading-none text-on-surface">{selectedCount}<span className="ml-1 text-xs font-normal text-on-surface-variant">개 파일</span></p>
          </div>
        )}

        <div className="mt-auto flex items-center gap-3 border-t border-outline-variant/20 px-4 pt-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-outline-variant/30 bg-surface-container-high/45 backdrop-blur-xl">
