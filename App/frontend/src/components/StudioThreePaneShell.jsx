...
export default function StudioThreePaneShell({
  ...
  useEffect(() => {
    const t = setTimeout(() => setEntranceOn(true), 36)
    return () => clearTimeout(t)
  }, [])

  return (
    <div className="relative flex h-screen min-h-0 w-full overflow-hidden px-2.5 pb-2.5 pt-1.5 text-on-surface sm:px-3 sm:pb-3 sm:pt-2">
      <div
        aria-hidden
        className={`pointer-events-none absolute inset-0 z-0 ${entranceOn ? 'studio-orb-focus-on' : 'studio-orb-focus-off'}`}
      />
      <div className="apple-studio-unified-canvas flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-[20px] sm:rounded-[24px]">
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          {/* ── 좌: 아이콘 + 라벨 내비 (대시보드형) ── */}
          <aside
            className={`apple-studio-sidebar-col relative flex w-[min(220px,22vw)] shrink-0 flex-col border-r border-white/[0.04] py-4 pl-3 pr-2 sm:w-[min(236px,24vw)] sm:py-5 sm:pl-3.5 sm:pr-2.5 ${
              entranceOn ? 'studio-stagger-item studio-stagger-in studio-stagger-d0' : 'studio-stagger-item studio-stagger-off'
            }`}
          >
            <div
          ...
          <div className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-transparent">
            ...
            <div
              className={`flex shrink-0 items-center justify-between gap-3 border-b border-white/[0.05] px-3 py-2.5 sm:px-5 ${
                entranceOn ? 'studio-stagger-item studio-stagger-in studio-stagger-d1' : 'studio-stagger-item studio-stagger-off'
              }`}
            ...
            <div className="flex min-h-0 min-w-0 flex-1 gap-3 overflow-hidden px-3 pb-3 pt-2 sm:gap-4 sm:px-5 sm:pb-4 sm:pt-3">
              ...
              <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
                <section
                  className={`apple-hero-card mb-3 shrink-0 rounded-[18px] p-4 sm:mb-4 sm:rounded-[22px] sm:p-6 ${
                    entranceOn ? 'studio-stagger-item studio-stagger-in studio-stagger-d2' : 'studio-stagger-item studio-stagger-off'
                  }`}
                >
                  {hero}
                </section>
                ...
                <div
                  className={`relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[18px] border border-white/[0.055] bg-white/[0.004] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-[72px] backdrop-saturate-105 sm:rounded-[20px] ${
                    entranceOn ? 'studio-stagger-item studio-stagger-in studio-stagger-d4' : 'studio-stagger-item studio-stagger-off'
                  }`}
                >
              ...
              {rightWidgets ? (
                <aside
                  className={`apple-studio-widgets hidden w-[min(280px,28%)] shrink-0 flex-col gap-3 overflow-y-auto pt-0.5 lg:flex ${
                    entranceOn ? 'studio-stagger-item studio-stagger-in studio-stagger-d5' : 'studio-stagger-item studio-stagger-off'
                  }`}
                >
                  {rightWidgets}
                </aside>
