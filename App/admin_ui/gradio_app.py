...
# ── UI 콜백 ─────────────────────────────────────────────────────────
def on_inspect(query, domain, top_n, use_lexical, use_asf):
    if not query.strip():
        return pd.DataFrame(), "", "쿼리를 입력하세요", None
    try:
        data = _api_inspect(query, domain, top_n, use_lexical, use_asf)
    except Exception as e:
        return pd.DataFrame(), "", f"오류: {e}", None

    rows = data["rows"]
    df = pd.DataFrame(rows)
    if not df.empty:
        cols = ["rank", "id", "dense", "lexical", "asf", "rrf", "confidence", "z_score"]
        df = df[[c for c in cols if c in df.columns]]
        for c in ("dense", "lexical", "asf", "rrf", "confidence", "z_score"):
            if c in df:
                df[c] = df[c].astype(float).round(4)
    summary = (
        f"도메인 {data['domain']} | 총 {data['total']}개 중 top-{data['returned']} 표시 | "
        f"μ_null={data['calibration']['mu_null']:.3f}, "
        f"σ_null={data['calibration']['sigma_null']:.3f}, "
...
def on_select(evt: gr.SelectData, df: pd.DataFrame, query: str, domain: str):
    if df is None or df.empty or evt.index is None:
        return [("항목을 선택하세요", None)], ""
    row_idx = evt.index[0] if isinstance(evt.index, list) else evt.index
    try:
        doc_id = df.iloc[row_idx]["id"]
    except Exception:
        return [("선택 오류", None)], ""
    try:
    ...
    info = f"**id**: `{doc_id}`\n\n**원본 경로**: `{data.get('source_path') or '(없음)'}`"
    if domain == "doc_page":
        info += f"  |  **페이지**: {data.get('page', 0)}"
    if data.get("matches"):
    ...
    with gr.Blocks(title="TRI-CHEF Admin Inspect", css=CSS) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                domain = gr.Radio(["doc_page", "image"], value="doc_page",
                                  label="도메인")
        with gr.Row():
            top_n = gr.Slider(50, 2000, value=200, step=50, label="상위 N 반환")
        ...
        with gr.Row():
            table = gr.Dataframe(label="전수 결과 (행 클릭 → 상세)",
                                 interactive=False, wrap=False, height=400)
        ...
        table.select(on_select, [table, query, domain], [detail, detail_info])
