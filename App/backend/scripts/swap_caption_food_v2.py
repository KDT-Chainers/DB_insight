"""scripts/swap_caption_food_v2.py — caption_food_v2 결과를 caption_3stage.json 에 swap.

  1. caption_3stage.json 자동 백업 → caption_3stage.bak_<timestamp>.json
  2. food 재캡션 결과(caption_food_v2_ckpt.json) 의 ID 만 부분 갱신
  3. SigLIP2 cosine 임베딩 캐시 도 갱신 필요 → 별도 단계 (rebuild_img_caption_embed.py)

⚠️ 사용자 명시 승인 후에만 실행. --dry-run 옵션 권장.

사용:
  python scripts/swap_caption_food_v2.py --dry-run     # 변경 미리보기
  python scripts/swap_caption_food_v2.py               # 실제 적용 (백업 포함)
  python scripts/swap_caption_food_v2.py --rollback    # 직전 백업으로 복원
"""
