"""scripts/sync_cat_to_triple.py — cat v2 캡션을 captions_triple.jsonl 에 동기화.
...
필요 (cache_img_Im_L1/L2/L3.npy 의 입력 소스).
  ...
  2. cat v2 결과(caption_cat_v2_ckpt.json)의 ID 만 in-place 갱신
  3. (별도) rebuild_im_cache_all.py --img-only 로 BGE-M3 재임베딩

사용:
  python scripts/sync_cat_to_triple.py --dry-run
  python scripts/sync_cat_to_triple.py
  python scripts/sync_cat_to_triple.py --rollback
"""
...
def cmd_sync(dry_run: bool):
    ...
    # cat: {id: {parts: {title, tagline, synopsis, tags_kr, tags_en}, caption: ...}}
    cat_map = {}
    ...
    logger.info(f"  cat v2 갱신 가능: {len(cat_map)}건")
    ...
    with jsonl.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if key in cat_map:
                ...
                # rebuild_im_cache_all.py 가 읽는 키 동일 갱신
                new_obj["L1"]      = title
    ...
    logger.info(f"  swap: {n_swapped}/{len(cat_map)}건")

    logger.info("\n  Sample 변경 (top 5):")
    for s in sample_changes:
        logger.info(f"    [{s['key'][:50]}]")
