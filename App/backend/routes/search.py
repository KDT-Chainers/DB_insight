...
def search():
    ...
    def _sort_key(r):
        ...
        # A (default, v21): 도메인별 가중치 + 파일명 매칭 보너스.
        # 정책:
        #   - Image / Doc:  primary = 0.35·유사도 + 0.55·정확도 + 0.10·신뢰도 + 파일명_보너스
        #     → "박스 속 고양이" 등 의미 컴포지션 쿼리에서 rerank가 박스 유무를 명확히 구분.
        #   - Video / Audio: primary = 0.65·유사도 + 0.25·정확도 + 0.10·신뢰도 + 파일명_보너스
        #     → AV 캡션·STT는 짧고 단편적이라 rerank가 단일 키워드에 편향됨.
        #       "코스모스 보이저호" 같은 다중 키워드 쿼리는 dense(E5+BGE-M3)가
        #       의미 결합을 더 안정적으로 잡음. v19 가중치 유지가 회귀 방지에 효과적.
        # 변경 이력:
        #   v19 → v20: 전 도메인 rerank 우선 (이미지 의미 쿼리 개선, 그러나 AV 다중키워드 회귀)
        #   v20 → v21: 도메인 분기로 양쪽 모두 최적화
