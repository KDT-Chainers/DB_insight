...
def search():
    ...
    def _sort_key(r):
        ...
        # A (default, v20): 옵션 1 — 정확도(rerank) 우선 + 파일명 매칭 보너스.
        # 정책:
        #   primary = 0.35·유사도 + 0.55·정확도 + 0.10·신뢰도 + 파일명_보너스(0~0.20)
        # 변경 사유 (v19 → v20):
        #   "박스 속에 있는 고양이" 등 의미적 컴포지션 쿼리에서, 유사도(dense)가 91~92%로
        #   포화돼 단순 "고양이" 이미지와 "박스 속 고양이"가 구별되지 않음. 반면 reranker는
        #   박스 유무를 23~84% 차로 명확히 구분 → rerank 가중치를 0.25→0.55, 유사도를
        #   0.65→0.35로 재배분하여 의미 정렬 품질 우선.
        # 파일명 보너스:
